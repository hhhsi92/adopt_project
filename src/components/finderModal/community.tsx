    ...
    <StyledForm>
      ...
      {selected.uuid && (
        <div className="selected">
          <em>선택된 항목</em>
          <a
            href={`/board/community/${selected.uuid}`}
            target="_blank"
            rel="noreferrer"
          >
            {selected.content}
...
export default function CommunityFinder(props: FinderProps) {
  ...
  useEffect(() => {
    ...
    const getPosts = async () => {
      try {
        const {
          data: { data },
        } = await api.get(`v1/adm/board/free?pageNo=${page}&pageSize=${pageSize}${filterQuery}`);
          ...
          <TableBody>
            {posts.length > 0 ? (
              posts.map((post) => {
                return (
                  <TableRow
                    key={`communityFinder_post_${post.uuid}`}
                    hover
